...
class UsuarioModel {
    ...
    // Solo acepta instalaciones
    public function obtenerUsuarioPorUsername($username, $user_type) {
        if ($user_type === 'instalacion') {
            $stmt = $this->conn->prepare("SELECT id, username, password, estado FROM usuarios_instalaciones WHERE username = ? AND tipo_usuario = 'ipd'");
        } else {
            return false;
        }
    ...
    public function registrarInstalacion($data) {
        $stmt = $this->conn->prepare("INSERT INTO usuarios_instalaciones (
            username, password, tipo_usuario, estado, email
        ) VALUES (?, ?, 'ipd', 1, ?)");
    ...
    if ($user_type === 'ipd') {
        $stmt = $this->conn->prepare(
            "SELECT id, username, email 
             ...
             WHERE email = ? AND estado = 1 AND tipo_usuario = 'ipd'"
        );
    } else {
        return false;
    }
