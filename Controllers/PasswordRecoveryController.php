 ...
 * SOLO para usuarios IPD (Institución Deportiva)
 */
...
class PasswordRecoveryController {
    ...
    public function solicitarRecuperacion($email, $user_type) {
        ...
        if ($user_type !== 'ipd') {
            return [
                'success' => false,
                'message' => 'Solo disponible para instituciones IPD.'
            ];
        }

        // Buscar usuario por email SOLO IPD
    ...
    public function cambiarPassword($token, $nueva_password, $confirmar_password) {
        ...
        // Actualizar contraseña solo si es IPD
        if ($tokenData['user_type'] !== 'ipd') {
            return [
                'success' => false,
                'message' => 'Solo permitido para usuarios IPD.'
            ];
        }
    ...
    public function prepararDatosEmailJS($email, $user_type) {
        ...
        // Buscar usuario solo en instalaciones IPD
        $usuario = $this->usuarioModel->buscarUsuarioPorEmail($email, $user_type);
        ...
        return [
            ...
            'email_data' => [
                ...
                'user_type_display' => 'Institución Deportiva' // Siempre texto claro para IPD
            ]
        ];
    }
