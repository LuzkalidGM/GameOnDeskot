...
class UserController {
    ...
    // Registro de deportista institucional IPD (sin PDF, solo datos básicos)
    public function registrarInstalacion() {
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            if (empty($form_data['username']) || empty($form_data['email'])) {
                ...
                // Forzamos tipo_usuario = 'ipd'
                $resultado = $this->model->registrarInstalacion([
                ...
                if (isset($resultado['error'])) {
                    ...
                    $success_message = "¡Registro exitoso! Ya puedes iniciar sesión.";
                    $form_data = array_map(fn($v) => '', $form_data);
                }
            }
        }
